class FilterEngine {
    constructor(rules = {}) {
        this.rules = {
            minFollowers: rules.minFollowers ?? 50, // Use ?? to allow 0
    ...
    /**
     * Evaluates a single lead against the defined rules.
     * @param {object} lead - The lead object to evaluate.
     * @param {object} [options] - Optional parameters.
     * @param {TwitterBot} [options.twitterBot] - A TwitterBot instance for dynamic checks like DM status.
     */
    async evaluateLead(lead, { twitterBot = null } = {}) {
        try {
            ...
            // NOTE: This adds significant time to filtering and increases rate-limit risk.
            // It should only be run if a twitterBot instance is provided.
            if (this.rules.preFilterCheckDMs && twitterBot) {
                const canDM = await twitterBot.checkDMButtonExists(lead.username);
                if (!canDM) {
                    return {
                        passed: false,
    ...
    calculateQualityScore(lead) {
        ...
        const qualityKeywords = this.rules.qualityScoreKeywords || [];
        const bioLower = bio.toLowerCase();
        const sportsMatches = qualityKeywords.filter(term => bioLower.includes(term)).length;
    ...
    async evaluateLeads(leads, { twitterBot = null } = {}) {
        ...
        for (const lead of leads) {
            try {
                const result = await this.evaluateLead(lead, { twitterBot });
                results.push({
                    lead,
                    result
                });
