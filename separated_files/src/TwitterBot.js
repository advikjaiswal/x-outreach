...
class TwitterBot {
    ...
    async initialize() {
        try {
            ...
            const proxies = this.config.scraping?.proxies || [];
            if (proxies.length > 0) {
    ...
    async login() {
        try {
            ...
            await usernameInput.fill(this.config.twitter.username);
            ...
            if (await disambiguationInput.isVisible()) {
                ...
                if (!this.config.twitter.phoneOrEmail) {
                ...
                await disambiguationInput.fill(this.config.twitter.phoneOrEmail);
                await this.page.click('button[role="button"]:has-text("Next")');
            }

            console.log("🔑 Entering password...");
            await passwordInput.waitFor({ state: 'visible', timeout: 15000 });
            await passwordInput.fill(this.config.twitter.password);
