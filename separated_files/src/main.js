...
class TwitterAutomationSystem {
    constructor(userConfig = {}) {
        ...
        // CRITICAL FIX: Merge the user-specific rate limits from the API call
        // with the default limits from the config file.
        dynamicConfig.rateLimits = { ...config.rateLimits, ...userConfig.rateLimits };
        ...
        if (this.isDemo) {
            ...
            this.twitterBot = new TwitterBot(dynamicConfig);
        }
    ...
    async runAutomationLoop() {
        ...
        while (!this.isShuttingDown) {
            ...
            try {
                // Check for working hours to appear more human
                if (!this.isDemo && !this.isWithinWorkingHours()) {
                    const sleepTime = 30 * 60 * 1000; // Check again in 30 minutes
                    const nextCheck = new Date(Date.now() + sleepTime).toLocaleTimeString();
                    console.log(`💤 Outside of configured working hours. Pausing until approximately ${nextCheck}.`);
                    await this.sleep(sleepTime);
                    continue; // Skip the rest of the cycle and re-evaluate on the next iteration
                }
    ...
    async filteringPhase() {
        ...
        try {
            ...
            for (const lead of rawLeads) {
                try {
                    // Pass the twitterBot instance only if not in demo mode and the rule is enabled
                    const evaluationOptions = {
                        twitterBot: (!this.isDemo && this.filterEngine.rules.preFilterCheckDMs) ? this.twitterBot : null
                    };
                    const filterResult = await this.filterEngine.evaluateLead(lead, evaluationOptions);
                    updates.push({ rowNumber: lead.id, status: filterResult.passed ? "PASS" : "FAIL", reason: filterResult.reason });

                    if (filterResult.passed) {
                        passedCount++;
                        console.log(`✅ @${lead.username}: PASS (Score: ${filterResult.score})`);
                    } else {
                        failedCount++;
    ...
    isWithinWorkingHours() {
        const behavior = config.engagement.humanBehavior;
        const now = new Date();
        const currentHour = now.getHours();

        // Check for working hours
        const isWorkingTime = currentHour >= behavior.workingHoursStart && currentHour < behavior.workingHoursEnd;
        if (!isWorkingTime) {
            console.log(`Current hour ${currentHour} is outside working hours (${behavior.workingHoursStart} - ${behavior.workingHoursEnd}).`);
            return false;
        }

        // Check for lunch break
        if (currentHour >= behavior.lunchBreakStart && currentHour < behavior.lunchBreakEnd) {
            console.log('🍽️  It is lunchtime. Pausing activity.');
            return false;
        }
